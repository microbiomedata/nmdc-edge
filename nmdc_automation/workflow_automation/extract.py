...
class wfXtraction:
    ...
    def extract(self):
        ...
        actid_directory = self.handler(
            self.claimedJobres["actid"], self.claimedJobres["workflow"]["id"]
        )
        ...
        if actid_directory is not None:
            for productKey, productValue in self.cromwellMetadata["outputs"].items():
                try:
                    print(
                        f"Worfklow Product {productValue} for {self.claimedJobres['workflow']['id']} being moved to {actid_directory}"
                    )
            ...
            sys.exit(
                "Process Has Been Mistakenly Repeated, Detected by Logic Capture. Goodbye"
            )
    ...
    def handler(self):
        ...
        try:
            ...
            for product in os.listdir(_path):
                print(
                    f"{product} exists for Activity {actid}, and {self.claimedJobres['config']['release']} is has not changed since last process"
                )
    ...
    def cromwell_metadata(self, expand=True):
        ...
        url = "%s/%s/metadata?expandSubWorkflows=%s" % (
            self.cromwell_root,
            self.job_id,
            expand_s,
        )
