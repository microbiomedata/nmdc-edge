...
function addSelector(mapdiv, mapid, dir){
	...
	selectdivdom.append($('<legend>Select a KEGG map:</legend>'));

	// KEGG map selector
	var pwytab = dir+'/exp_pathway.txt';
	...
	$.when( loadEXPPWY ).done(function ( v1, v2 ) {
...
function drawMap(mapdiv, mapid, showicon, org, dir) {
	mapid      = mapid;                               //initial map
	showicon   = showicon || 2;                       //show number of genes icon if number of genes > gene_show
	org        = org || "ec";                         //default organism
	var mapxml = dir+'/'+org+mapid+'.xml';
	var expjson = dir+'/'+org+mapid+'.anno.json';
	var mapImg;
	var kgml;
	...
	mapImg.attr("src", dir+"/"+org+mapid+".png");
	...
	mapImg.load(function(){
		...
		titledom.html("Pathway: "+mapid+note);
				...
				$(kgml).find( "graphics" ).each(function(){
					...
					if ( entry_type == "enzyme" ){
						...
						$.each(ids, function(index, id){
                            jdom.addClass("enzyme-area-target");
                            jdom.html(id);
							//add annotation gene number icon
							if( $(annojson[id].anno).length ){
								var x=jdom.width()-6;
								var y=-8;
								contentdom = tranIcon;
								var num = contentdom.html();
								var tolnum = parseInt(num) + Object.keys(annojson[id].anno).length;
							...
							$.each( pwys, function(idx, pwy){
								$('<a data-ajax="false" style="margin-right:0.5em">')
									.html(pwy)
									.on("click",function(){ if(pwy!=mapid){$('#pathway-selector').val(pwy).change()}})
									.appendTo( pathwayTd );
							});
							...
							$('<tr>')
								...
								.appendTo(tbodydom);
							$('<tr>')
								.append( $('<td>').html('Product') )
								.append( $('<td>').html( annojson[id].info.product ) )
                                .appendTo(tbodydom);
							$('<tr>')
								.append( $('<td>').html('Class') )
								.append( $('<td>').html( annojson[id].info.class ) )
                                .appendTo(tbodydom);
							$('<tr>')
								.append( $('<td>').html('Reaction') )
								.append( $('<td>').html( annojson[id].info.reaction ) )
                                .appendTo(tbodydom);
							$('<tr>')
								.append( $('<td>').html('Comment') )
								.append( $('<td>').html( annojson[id].info.comment ) )
                                .appendTo(tbodydom);
							...
							//add annotation
							if( $(annojson[id].anno).length ){
								tooltipcontent.append($('<span>').html("Gene annotation ("+Object.keys(annojson[id].anno).length+")"));

								var tabledom = $('<table data-role="table" data-mode="reflow" class="gene-info ui-responsive">');
								var theaddom = $('<thead>');
                                $('<tr>').append( $('<th>').html('Contig / Locus') )
                                        .append( $('<th>').html('Type') )
								...
								var tbodydom = $('<tbody>');
								$.each(annojson[id].anno,function(gene,ginfo){
									var url_gene = '<a href="https://www.ncbi.nlm.nih.gov/gene/?term='+ginfo.gene+'" target="_new">'+ginfo.gene+'</a>';
									$('<tr>').append( $('<td>').html( ginfo.contig + " / " + ginfo.locus_tag ) )
                                            .append( $('<td>').html( ginfo.type ) )
                                            .append( $('<td>').html( ginfo.start + " - " + ginfo.end + " (" + ginfo.strand + ")" ) )
                                            .append( $('<td>').html( url_gene ) )
                                            .append( $('<td>').html( ginfo.prod ) )
                                            .appendTo(tbodydom);
								});
					...
					//dealing with maps graphics //skip the title block of current map
						...
						if( $('#pathway-selector option[value="'+mapid+'"]').length ){
							...
							tooltipcontent
								...
								.append( $('<img src="'+dir+"/"+org+mapid+'.png" style="width: 500px; height: auto"></img>') );
						}
						else{
							tooltipcontent
								.append( $('<span class="tag-label">'+mapid+": "+name+'</span>') )
